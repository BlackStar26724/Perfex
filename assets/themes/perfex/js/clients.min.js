...
function func_send_order() {
    var order_qty = document.getElementsByName("order_qty");
    var data = [];
    var user_id = document.getElementsByName("user_id");
    user_id = user_id[0].getAttribute('value');
    var flag = 0;
    for (var i = order_qty.length - 1; i >= 0; i--) {
        if(order_qty[i].value != 0 || order_qty[i].value != ''){
            var obj = {};
            obj['item_id'] = order_qty[i].getAttribute('value1');
            obj['qty'] = order_qty[i].value;
            data.push(obj);
            flag = 1;
        }
    }
    if(flag == 1) {
        $.ajax({
            ...
            url: site_url + 'clients/get_order/',
            success: function(data){
                alert(data);
                order_qty = document.getElementsByName("order_qty");
                for (var i = order_qty.length - 1; i >= 0; i--) {
                    if(order_qty[i].value != 0 || order_qty[i].value != ''){
                        order_qty[i].value = '';
                    }
...
function category_select(selectObject) {
  var filter, table, tr, td, i, txtValue;
  if(selectObject.value == "All") {
    filter = ""; 
  }
  else {
    filter = selectObject.value.toUpperCase();
  }
