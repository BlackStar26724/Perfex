...
$(function () {
    ...
    $("body").on('change', '.f_client_id #clientid', function () {
        ...
        requestGetJSON('invoices/client_change_data/' + val + '/' + currentInvoiceID).done(function (response) {
            ...
            init_billing_and_shipping_details();
            document.getElementById('client_group').setAttribute('value', response['group_name']);
...
function add_item_to_preview(id) {
    requestGetJSON('invoice_items/get_item_by_id/' + id).done(function (response) {
        ...
        var vip_info = document.getElementById('client_group').getAttribute('value');
        if(vip_info == 'T1') {
            vip_info = 25;
        }
        else if(vip_info == 'T2') {
        ...
        var client_group = $("body").find('.main select[name="vip"]');
        client_group.val(vip_info)
        client_group.selectpicker('refresh');
...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
    ...
    var amount = data.rate * data.qty * (100 - data.discount) / 100;
    ...
    get_taxes_dropdown_template(tax_name, data.taxname).done(function (tax_dropdown) {
        ...
        table_row += '<td class="vip"><select name="newitems[' + item_key + '][vip]" onblur="calculate_total();" onchange="calculate_total();" data-discount class="selectpicker vip" data-width="100%" data-none-selected-text="">';

        if(data.discount == 0) {
            selected1 = 'selected';
        }
        else if(data.discount == 25) {
            selected2 = 'selected';
        }
        else if(data.discount == 35) {
            selected3 = 'selected';
        }
...
function get_item_preview_values() {
    ...
    response.discount = $('.main select[name="vip"]').val();
...
function calculate_total() {
    ...
    $.each(rows, function () {
        ...
        discount = $(this).find('[data-discount]').val();
        if(discount == '0%') {
            discount = 0;
        }
        else if(discount == '25%') {
        ...
        _amount = accounting.toFixed($(this).find('td.rate input').val() * quantity * (100 - discount) / 100, app.options.decimal_places);
